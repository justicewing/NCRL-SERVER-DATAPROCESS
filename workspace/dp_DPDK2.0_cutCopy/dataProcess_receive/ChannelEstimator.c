...
void chestDCT_init(struct chestDCT_t *chestDCT_p, int CarrierNum, int inter_freq, int SymbNum){
	...
	chestDCT_p->PilotFreqIndex = (int*)malloc(sizeof(int)*PilotFreqNum);
	chestDCT_p->PG_i = (float*)malloc(sizeof(float)*PilotFreqNum);
	chestDCT_p->PG_est = (float*)malloc(sizeof(float)*PilotFreqNum);
	chestDCT_p->SignalRecPilot = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	chestDCT_p->Pilot_nTx = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	chestDCT_p->CFR_est_Pilot = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	chestDCT_p->h_dct = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	chestDCT_p->CFR_est_nCh = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*CarrierNum*SymbNum);
}
void chestDCT_free(struct chestDCT_t *chestDCT_p){
	free(chestDCT_p->PilotFreqIndex);
	free(chestDCT_p->PG_i);
	free(chestDCT_p->PG_est);
	free(chestDCT_p->SignalRecPilot);
	free(chestDCT_p->Pilot_nTx);
	free(chestDCT_p->CFR_est_Pilot);
	free(chestDCT_p->h_dct);
	free(chestDCT_p->CFR_est_nCh);
}
void ChannelEstimator_DCT(lapack_complex_float *SignalRec, lapack_complex_float *PilotSymb, float *R_DCT, int *PilotSymbIndex, int TxAntNum, int RxAntNum, int CarrierNum, int UserNum, int MaxBeam, int PilotSymbNum, int SymbNum, int inter_freq, float sigma, lapack_complex_float *CFR_est, struct chestDCT_t *chestDCT_p)
{
	...
	PilotFreqNum = CarrierNum / inter_freq;
	PilotFreqIndex = chestDCT_p->PilotFreqIndex;
	PG_i = chestDCT_p->PG_i;
	PG_est = chestDCT_p->PG_est;
	SignalRecPilot = chestDCT_p->SignalRecPilot;
	Pilot_nTx = chestDCT_p->Pilot_nTx;//
	CFR_est_Pilot = chestDCT_p->CFR_est_Pilot;//
	h_dct = chestDCT_p->h_dct;//
	CFR_est_nCh = chestDCT_p->CFR_est_nCh;
...
void Cal_DCT2(lapack_complex_float* DCTMatrix, lapack_complex_float* Matrix, int M) {
	...
	if (M == 1) {
		...
		status = DftiComputeForward(my_desc1_handle, fft_1);
		status = DftiComputeForward(my_desc1_handle, fft_2);
