...
extern sem_t exit_signal;
const int paraNum_tx = 2;
const int paraNum_rx = 5;
const int packCache = 20;
const int SBNum = 50;
//test
...
void TaskScheduler_tx(void *arg){
	...
	for (int i = 0; i < paraNum_tx; i++) ServiceEN_tx[taskNum_tx * i] = 1;
	if(TIME_EN == 1) gettimeofday(&tx_begin, NULL);//--------------------rx
	while(1){
		//sleep(1);
		...
		for(int n = 0; n < paraNum_tx; n++){
			...
			//  1.1  crc attach - code block segmentation
			if(packNum < packCache && ServiceEN_tx[n * taskNum_tx] == 1){//有数据需要发送，且任务添加器(n,1)打开
			...
			//  1.2  crc attach - turbo coding - rate matching - code block concatenation
			if(ServiceEN_tx[n * taskNum_tx + 1] == LayerNum){//任务添加器(n,2)打开
				int s = 0;
				cbtaskNum[n] = 0;
				for (int i = 0; i < LayerNum; i++){
					...
					for (int r = 0; r < (cb_tx[n][i]->C); ++r){
						...
						if(r < cb_tx[n][i]->C - Lamda) rm_outlen_tx[n][j] = CQI_mod[CQI_index[n * MaxBeam + i]] * (max_symbol_num / cb_tx[n][i]->C);
						else rm_outlen_tx[n][j] = CQI_mod[CQI_index[n * MaxBeam + i]] * ( (max_symbol_num - 1) / cb_tx[n][i]->C + 1);
			...
			//  1.3  packing
			if(ServiceEN_tx[n * taskNum_tx + 2] == cbtaskNum[n] && cbtaskNum[n] != 0){//任务添加器(n,3)打开
				if(TIME_EN == 1){
					TIME++;
				...
				if(count_ms == 0){
					SNR = SNR + STEP;
					if(SNR > SNR_MAX) SNR = SNR_MIN;
					//printf("\nSNR = %f\n",SNR);
					//srand(seed);
				}
				count_ms++;
				if(count_ms == testError) count_ms = 0;
				m = 0;
				k = 0;
				int s = 0;
				for(int ns = 0; ns < SymbolNum; ns++){
				...
				if(TIME_EN == 1){//---------------------Throughput information
						//TIME++;
						for(int i = 0; i < LayerNum; i++) bitsNum += data_len_tx[n][i];
						if(TIME == testTime){
...
void TaskScheduler_rx(void *arg){
	...
	while(1){
		...
		for(int n = 0; n < paraNum_rx; n++){
			...
			//  1.2  channel estimating - signal detecting
			if(ServiceEN_rx[n * taskNum_rx] == LayerNum && readyNum != 0){//有数据需要处理，且任务添加器(n,1)打开
				if(readyNum > 1) pack = p;
				else pack = 0;
				for(int i = 0; i < LayerNum; ++i){
			...
			if(ServiceEN_rx[n * taskNum_rx + 1] == SBNum){
				...
				for (int i = 0; i < LayerNum; i++){
					...
					for (int r = 0; r < (cb_rx[n][i]->C); ++r){
						int j = i * cbNum + r;
						int jp = n * LayerNum * cbNum + i * cbNum + r;
						if(r < cb_rx[n][i]->C - Lamda) rm_outlen_rx[n][i] = CQI_mod[CQI_index[n * MaxBeam + i]] * (max_symbol_num / cb_rx[n][i]->C);
						else rm_outlen_rx[n][i] = CQI_mod[CQI_index[n * MaxBeam + i]] * ( (max_symbol_num - 1) / cb_rx[n][i]->C + 1);
						...
						if( (cb_rx[n][i]->C) > 1){
							if(r == 0) c += Kr - cb_rx[n][i]->F - crc_length;
							else c += Kr - crc_length;
						}
			...
			if(BERsignal[n] == LayerNum){
				if(statistics == 1){//----------------------Statistics information
					if(TIME_EN == 1){
						TIME++;
					...
					if(BLER_EN == 1 || BER_EN == 1){//------Error information
						count_ms++;
						if(count_ms == testError){
							printf("\n--------------------No.%d~%d--------------------",runIndex + 2 - testError,runIndex + 1);
							count_ms = 0;
							if(BLER_EN == 1){//-------------Block error information		
							...
							if(BER_EN == 1){//--------------Bits error information
					...
					if(TIME_EN == 1){//---------------------Throughput information
						//TIME++;
						for(int i = 0; i < LayerNum; i++) bitsNum += data_len_rx[n][i];
						if(TIME == testTime){
							//gettimeofday(&rx_end, NULL);//______________________rx
							//runtime.rx += (rx_end.tv_sec - rx_begin.tv_sec) + (rx_end.tv_usec - rx_begin.tv_usec) / 1000000.0;
							printf("\n--------------------RX : Time statistics--------------------\n");
				...
				pthread_mutex_lock(&mutex_packNum);
				if(test_rx[n].packIdx != 0) packNum--;
				pthread_mutex_unlock(&mutex_packNum);
