...
void chestLS_init(struct chestLS_t *chestLS_p, int CarrierNum, int inter_freq, int SymbNum){
	...
	chestLS_p->PilotFreqIndex = (int*)malloc(sizeof(int)*PilotFreqNum);
	chestLS_p->SignalRecPilot = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	chestLS_p->Pilot_nTx = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	chestLS_p->CFR_est_Pilot = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	chestLS_p->h_dct = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	chestLS_p->CFR_est_nCh = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*CarrierNum*SymbNum);
	chestLS_p->TrigoMx = (float*)malloc(sizeof(float)*PilotFreqNum*PilotFreqNum);
}
void chestLS_free(struct chestLS_t *chestLS_p){
	free(chestLS_p->PilotFreqIndex);
	free(chestLS_p->SignalRecPilot);
	free(chestLS_p->Pilot_nTx);
	free(chestLS_p->CFR_est_Pilot);
	free(chestLS_p->h_dct);
	free(chestLS_p->CFR_est_nCh);
	free(chestLS_p->TrigoMx);
}
float ChannelEstimator_LS(lapack_complex_float *SignalRec, lapack_complex_float *PilotSymb, int *PilotSymbIndex, int TxAntNum, int RxAntNum, int CarrierNum, int PilotSymbNum, int SymbNum, int inter_freq, lapack_complex_float *CFR_est, struct chestLS_t *chestLS_p)
{
	...
	PilotFreqNum = CarrierNum / inter_freq;
	PilotFreqIndex = chestLS_p->PilotFreqIndex;
	SignalRecPilot = chestLS_p->SignalRecPilot;
	Pilot_nTx = chestLS_p->Pilot_nTx;//
	CFR_est_Pilot = chestLS_p->CFR_est_Pilot;//
	h_dct = chestLS_p->h_dct;//
	CFR_est_nCh = chestLS_p->CFR_est_nCh;
	TrigoMx = chestLS_p->TrigoMx;
			...
			*/
			...
			for (p = 0; p < PilotSymbNum; ++p) {
				...
				for (k = PilotFreqNum - NoiseRow; k < PilotFreqNum; ++k) {
					...
					//printf("\n%f+%fi ",h_dct[k].real,h_dct[k].imag);
					//sleep(1);
				}
