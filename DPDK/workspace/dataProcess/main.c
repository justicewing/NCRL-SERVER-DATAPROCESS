...
int package(, struct rte_mbuf *m)
{
	...
	int cnt = 0;
		...
		// FILE *file_test = fopen("test.txt", "w+");
		for (cnt = 0; cnt < uhdr.length - 8; cnt++)
		{
			*adcnt = data[cnt];
			// fprintf(file_test, "%d,%d%,02X,%02X\n", data[cnt], *adcnt, data[cnt], *adcnt);
			*adcnt++;
		}
		// fclose(file_test);
