...
void pool_init(int coreId_start, int _threadNum, int pool_index)  // 线程起始序号,线程数，线程池序号
{  
	int j = pool_index;
...
{  
	...
	struct Task* head = pool[j] -> taskHead;  
	if(head == NULL)  
