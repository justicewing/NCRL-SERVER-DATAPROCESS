...
{	
	...
	while (!force_quit) 
	{
		...
		if ( unlikely( ( 1 ) && (diff_tsc > drain_tsc) ) )
		{
			portid = 0;
			...
			if (timer_period > 0)
			{
				...
				/* do this only on master core */
					...
					printf("delay= %f us\n",(double)running_second/port_statistics[portid].rx*1000000.0);
					printf( "pack_err= %d\n", pack_err );	
					/* reset the timer */
					timer_tsc = 0;
				}
	...
	{	
		...
		//{
			for (j = 0; j < nb_rx; j++) 
			{
				...
				sent = rte_eth_tx_buffer(portid, 0, buffer,pkts_burst[j]);
				port_statistics[portid].tx += sent;
				
				
				//while( (!force_quit) && (rte_ring_mp_enqueue(ring_send,pkts_burst[j])<0));		
