...
void gencons(float *consRe, float *consIm, int order, float SNR) {
	...
	double rou, p;
	double sigma = sqrt(pow(10,SNR/10));
	switch (order){
	case 2:
		for(int i = 0; i < 2048; ++i) {
			rou = gaussrand() / sigma;
			p = rand() / (double)(RAND_MAX) * 2 * PI - PI;
			consRe[i] = Map4QAM[rand() % 2] + rou * cos(p);
			consIm[i] = Map4QAM[rand() % 2] + rou * sin(p);
		}
		break;
	case 4:
		for(int i = 0; i < 2048; ++i) {
			rou = gaussrand() / sigma;
			p = rand() / (double)(RAND_MAX) * 2 * PI - PI;
			consRe[i] = Map16QAM[rand() % 4] + rou * cos(p);
			consIm[i] = Map16QAM[rand() % 4] + rou * sin(p);
		}
		break;
	case 6:
		for(int i = 0; i < 2048; ++i) {
			rou = gaussrand() / sigma;
			p = rand() / (double)(RAND_MAX) * 2 * PI - PI;
			consRe[i] = Map64QAM[rand() % 8] + rou * cos(p);
			consIm[i] = Map64QAM[rand() % 8] + rou * sin(p);
		}
		break;
	case 8:
		for(int i = 0; i < 2048; ++i) {
			rou = gaussrand() / sigma;
			p = rand() / (double)(RAND_MAX) * 2 * PI - PI;
			consRe[i] = Map256QAM[rand() % 16] + rou * cos(p);
			consIm[i] = Map256QAM[rand() % 16] + rou * sin(p);
		}
		break;
	default:
		break;
...
void mmwitfpack_rx(uint8_t *conste, uint8_t *channel, double thput){
	...
	for(int i = 0; i < 2048; ++i){
		...
		sendbuf.constellation[i][0] = (float)fix_16 / copoint;//I路
		fix_16 = (conste[j+2]<<8) + conste[j+3];
		sendbuf.constellation[i][1] = (float)fix_16 / copoint;//Q路
		j = j + 4;
	}
	//信道1/H
...
{	
	...
	while (!force_quit)
	{
		...
		//{	
			...
			gencons(consRe, consIm, moduType, 30);
			getchan(Magn, Phase, HRe, HIm, chcnt);
			chcnt++;
			if(chcnt == 1000) chcnt = 0;
			
			for(int i = 0; i < 2048; ++i){
				sendbuf.constellation[i][0] = consRe[i];//I路
				sendbuf.constellation[i][1] = consIm[i];//Q路
				sendbuf.chan[i][0] = Magn[i];//幅度
				sendbuf.chan[i][1] = Phase[i];//相位
			}
			sendbuf.rethrou = 0.321 *moduType / 2 + 0.321 *moduType / 2 * 0.1 * (rand() / (double)(RAND_MAX)-0.5);
					...
					if(pack_error) {
		   			...
		   			//if(data_type == 0x10){ //如果是信道，读取数据后显示
						...
						//if(flag) {printf("flag");print_datapack(data_pack_Rx[data_idx]); flag = 0;}
		   				//mmwitfpack_rx(data_pack_Rx[data_idx].data, data_pack_Rx[data_idx].data + 8400, rate_to_display);

						gencons(consRe, consIm, 4, 2);
						getchan(Magn, Phase, HRe, HIm, chcnt);
						chcnt++;
						if(chcnt == 1000) chcnt = 0;
						...
						sendbuf.rethrou = rate_to_display;
