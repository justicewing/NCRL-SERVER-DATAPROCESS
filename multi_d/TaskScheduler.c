...
void TaskScheduler_tx(void *arg)
{
	...
	{
		package_tx[p].tbs = (int *)malloc(sizeof(int) * MAX_BEAM);
		package_tx[p].CQI_index = (int *)malloc(sizeof(int) * MAX_BEAM);
	...
	int *CQI_index = (int *)malloc(sizeof(int) * MAX_BEAM * CacheNum_tx);
			...
			{ //任务添加器(n,3)打开
				...
				for (int ns = 0; ns < SYMBOL_NUM; ns++)
				{
					for (int nc = 0; nc < CARRIER_NUM; nc++)
					{
						...
						for (int i = 0; i < LayerNum; i++)
						{
							x_temp[i] = x[index_write_tx][CARRIER_NUM * LayerNum * ns + LayerNum * nc + i];
						}
						cblas_cgemm(Layout, transa, transb, _m, _n, _k, &alpha, H_temp, lda, x_temp, ldb, &beta, y_temp, ldc);
						for (int i = 0; i < RX_ANT_NUM; i++)
						{
							y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].real = y_temp[i].real + sigma * gwn[CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].real;
							y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].imag = y_temp[i].imag + sigma * gwn[CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].imag;
							//if(nc == 144) printf("\nRX %d symbol %d Carrier %d : y_temp = %f+%fi, y = %f+%fi, noise = %f+%fi\n",i,ns,nc, y_temp[i].real,y_temp[i].imag,y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].real,y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].imag,y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].real - y_temp[i].real,y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].imag - y_temp[i].imag);
							E += y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].real * y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].real + y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].imag * y[index_write_tx][CARRIER_NUM * RX_ANT_NUM * ns + RX_ANT_NUM * nc + i].imag;
				...
				for (int i = 0; i < LayerNum; i++)
				{
					...
					package_tx[index_write_tx].CQI_index[i] = CQI_index[n * MAX_BEAM + i];
		...
		for (int n = 0; n < paraNum_rx; n++)
		{
			...
			{ //有数据需要处理，且任务添加器(n,1)打开
				...
				for (int i = 0; i < LayerNum; ++i)
				{
					...
					CQI_index[n * MAX_BEAM + i] = package_rx[index_read_rx].CQI_index[i];
