...
void REstimator_init(struct Rest_t *Rest_p, int CarrierNum, int inter_freq, int SymbNum){
	int PilotFreqNum = CarrierNum / inter_freq;
	
	Rest_p->PilotFreqIndex = (int*)malloc(sizeof(int)*PilotFreqNum);
	Rest_p->SignalRecPilot = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	Rest_p->Pilot_nTx = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	Rest_p->CFR_est_Pilot = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	Rest_p->h_dct = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*PilotFreqNum);
	Rest_p->R_tmp = (float*)malloc(sizeof(float) * PilotFreqNum * SymbNum);
}
void REstimator_free(struct Rest_t *Rest_p){
	free(Rest_p->PilotFreqIndex);
	free(Rest_p->SignalRecPilot);
	free(Rest_p->Pilot_nTx);
	free(Rest_p->CFR_est_Pilot);
	free(Rest_p->h_dct);
	free(Rest_p->R_tmp);
}
float REstimator(lapack_complex_float *SignalRec, lapack_complex_float *PilotSymb, int *PilotSymbIndex, int TxAntNum, int RxAntNum, int CarrierNum, int SymbNum, int PilotSymbNum, int inter_freq, float *R_est, struct Rest_t *Rest_p)
{
	...
	PilotFreqIndex = Rest_p->PilotFreqIndex;
	SignalRecPilot = Rest_p->SignalRecPilot;
	Pilot_nTx = Rest_p->Pilot_nTx;
	CFR_est_Pilot = Rest_p->CFR_est_Pilot;
	h_dct = Rest_p->h_dct;
	R_tmp = Rest_p->R_tmp;
