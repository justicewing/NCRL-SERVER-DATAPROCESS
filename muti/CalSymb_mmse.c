...
void print_mx(lapack_complex_float *mx, char *name, int M, int N);

void CalSymb_init(struct CalSymb_t *CalSymb_p, int LayerNum, int RxAntNum){
	CalSymb_p->x_est = (lapack_complex_float *)malloc(sizeof(lapack_complex_float)*LayerNum);
	CalSymb_p->x_var = (float *)malloc(sizeof(float)*LayerNum);
	CalSymb_p->Signal_Rec = (lapack_complex_float *)malloc(sizeof(lapack_complex_float)*RxAntNum);
	CalSymb_p->ch_mx = (lapack_complex_float *)malloc(sizeof(lapack_complex_float)*(RxAntNum*LayerNum));
	CalSymb_p->ch_res = (lapack_complex_float *)malloc(sizeof(lapack_complex_float)*RxAntNum*LayerNum);
	
	CalSymb_p->ch_q = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*(RxAntNum + LayerNum)*LayerNum);
	CalSymb_p->ch_q1 = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*RxAntNum*LayerNum);
	CalSymb_p->ch_q2 = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*LayerNum*LayerNum);
	CalSymb_p->Q_Matrix = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*LayerNum*RxAntNum);
	CalSymb_p->lamda = (float *)malloc(sizeof(float)*LayerNum);
	CalSymb_p->tau = (lapack_complex_float*)malloc(sizeof(lapack_complex_float)*LayerNum);
}
void CalSymb_free(struct CalSymb_t *CalSymb_p){	
	free(CalSymb_p->x_est);
	free(CalSymb_p->x_var);
	free(CalSymb_p->Signal_Rec);
	free(CalSymb_p->ch_mx);
	
	free(CalSymb_p->ch_q);
	free(CalSymb_p->ch_q1);
	free(CalSymb_p->ch_q2);
	free(CalSymb_p->lamda);
	free(CalSymb_p->tau);
}
void CalSymb_mmse(lapack_complex_float *h_tmp, lapack_complex_float *Signal, int RxAntNum, int CarrierNum, int LayerNum, int SymbNum, float sigma, int Step, int flg_ave, lapack_complex_float *SymbEst,float *SymbVar, float *SINR_est, struct CalSymb_t *CalSymb_p)
{
	...
	x_est = CalSymb_p->x_est;
	x_var = CalSymb_p->x_var;
	Signal_Rec = CalSymb_p->Signal_Rec;
	ch_mx = CalSymb_p->ch_mx;
	ch_res = CalSymb_p->ch_res;

	ch_q = CalSymb_p->ch_q;
	ch_q1 = CalSymb_p->ch_q1;
	ch_q2 = CalSymb_p->ch_q2;
	Q_Matrix = CalSymb_p->Q_Matrix;
	lamda = CalSymb_p->lamda;
	tau = CalSymb_p->tau;
...
void print_mx(lapack_complex_float *mx, char *name, int M, int N) {
	...
	for (i = 0; i < M; i++) {
		for (j = 0; j < N; ++j) {
			printf("%f+%fi\t", mx[j*M + i].real, mx[j*M + i].imag);
		}
		printf("\n");
	}
}
